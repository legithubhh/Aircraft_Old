...
/* Private types -------------------------------------------------------------*/
/* Private variables ---------------------------------------------------------*/
static const float xb[3] = {1, 0, 0};
static const float yb[3] = {0, 1, 0};
static const float zb[3] = {0, 0, 1};
...
void INS_Init(void)
{
    ...
    // imu heat init
    TempCtrl.Set(1000, 20, 0, 1700, 300, 0, 200);
...
{
    float a = INS.Pitch;
    INS.Pitch = INS.Roll - pitch_offset;
    INS.Roll = a - roll_offset;
    INS.Yaw = INS.Yaw - yaw_offset;
}
...
{
    TempCtrl.ref = RefTemp;
    TempCtrl.fdb = BMI088.Temperature;
    TempCtrl.NormalCalc();
    __HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, TempCtrl.output);
}
